...
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  Layer 2: 愿景守护 — Review 回读需求                             │
│  ───────────────────────────────────                            │
│  • 每次 review 必须附 ≤5 行原始需求摘要                          │
│  • 防止"做完了但不是用户要的"                                    │
│  • 冷启动验证器：全新 Agent 只看需求+交付物                       │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  Layer 1: 信息分层 — 按需加载                                    │
│  ─────────────────────────────                                  │
│  • CLAUDE.md 不是百科全书，越少越精准                            │
│  • 分层加载：身份层 ≤100 行 + Skill 按需加载                     │
│  • 避免"有噪声无信号"                                            │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  Layer 0: 知识编码 — 描述是路由信号                              │
│  ─────────────────────────────────                              │
│  • Skill description 不是营销文案，是分类器                      │
│  • 必须有：Use when + Not for + Output                           │
│  • 写不好 = 正文永远不会被加载                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 12.9.3 Skill Description 三件套

```yaml
description: >
  处理 reviewer 反馈：修复 + 技术论证。
...
| **Not for** | 告诉路由器"看起来像但别选我"——边界请求时防误触发 |
| **Output** | 钉死交付物，减少漂移空间 |

### 12.9.4 冷启动验证器

```
问题：参与实现的 Bot 都"在上下文里"→ 无法看到交付物的缺陷

解决：
起一个全新的 Agent session，只给它：
1. 需求文档
2. 交付物截图

不给：实现过程、review 历史、上下文
...
│  热层 (Hot) — 活跃任务索引                                       │
│  ─────────────────────────                                      │
│  • BACKLOG.md：只放活跃 Feature，~10 行                          │
│  • 完成后移除，聚合文件永久保留                                   │
│  • 一眼扫完就知道项目在做什么                                     │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  温层 (Warm) — Feature 聚合入口                                  │
│  ─────────────────────────────────                              │
│  • 每个 Feature 一个聚合文件 docs/features/Fxxx-name.md          │
│  • 散落文档的导航入口                                             │
│  • 问"F21 什么情况"？打开一个文件就够了                           │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  冷层 (Cold) — 原始文档                                          │
│  ─────────────────────────                                      │
│  • plans/、discussions/、mailbox/、bug-report/                   │
│  • 原始记录，通过 frontmatter 挂接到 Feature                      │
│  • 定期归档到 archive/YYYY-MM/                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 12.10.2 Frontmatter 契约

```yaml
---
feature_ids: [F041]           # 关联的 Feature ID（强引用）
doc_kind: discussion          # plan|discussion|research|bug-report|mailbox|decision
created: 2026-03-08           # 创建时间
---
```
...
| 字段 | 作用 |
|------|------|
| `feature_ids` | 这个文档跟哪个 Feature 有关 → 可机器检索 |
| `doc_kind` | 这是什么类型的文档 → 枚举值，不是自由文本 |
| `created` | 什么时候建的 → 时间线排序 |

**关键决策**：`stage`（Feature 的进度状态）**不下沉到普通文档**

```
如果 100 个散落文档都有 stage: in-progress
Feature 状态变了要跑 20 个文件里改 → 蜘蛛网 2.0

正确做法：
- 状态只在一处记录（聚合文件）
- 散落文档只记录"跟谁有关"（feature_ids）
```

### 12.10.3 Lessons Learned 模板
...
Token 数量不重要：
- 549 行的 CLAUDE.md 不如 109 行的精准版本
- 76 个测试不如 1 个"打开产品看看对不对"
- 14 轮 review 不如 5 行原始需求摘要
```

**知识管理原则**:
- 热层只放活跃项（~10 行）
- 温层是聚合入口（一个 Feature 一个文件）
- 冷层是原始记录（加 frontmatter 挂接）
- 定期归档，保持信噪比
