...
export const initDb = () => {
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS agent_mappings (
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS bot_stats (
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS oneapi_channels (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS api_key_model_quotas (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  db.exec(`CREATE INDEX IF NOT EXISTS idx_model_quotas_api_key ON api_key_model_quotas(api_key)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_model_quotas_model ON api_key_model_quotas(model_name)`);

  // API Key Usage Logs table
  db.exec(`CREATE TABLE IF NOT EXISTS api_key_usage_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    api_key TEXT NOT NULL,
    model_name TEXT,
  ...
  db.exec(`CREATE INDEX IF NOT EXISTS idx_usage_logs_api_key ON api_key_usage_logs(api_key)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_usage_logs_created_at ON api_key_usage_logs(created_at)`);

  // World Info table for dynamic context injection
  db.exec(`CREATE TABLE IF NOT EXISTS world_info (
    id TEXT PRIMARY KEY,
  ...
  db.exec(`CREATE INDEX IF NOT EXISTS idx_world_info_room ON world_info(room_id)`);
