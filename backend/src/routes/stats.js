...
export const statsRoutes = async (fastify, options) => {
  ...
  fastify.get('/api/stats', async (request, reply) => {
    try {
      ...
      const totalAgents = getCount('SELECT COUNT(*) as count FROM bots');
      const activeAgents = getCount("SELECT COUNT(*) as count FROM bots WHERE status = 'online'");
      const totalMessages = getCount('SELECT COUNT(*) as count FROM messages');
      const todayMessages = getCount("SELECT COUNT(*) as count FROM messages WHERE date(timestamp) = date('now')");
  ...
  fastify.get('/api/stats/dashboard', async (request, reply) => {
    try {
      ...
      const totalMessages = getCount('SELECT COUNT(*) as count FROM messages');
      const todayMessages = getCount("SELECT COUNT(*) as count FROM messages WHERE date(timestamp) = date('now')");
      const yesterdayMessages = getCount("SELECT COUNT(*) as count FROM messages WHERE date(timestamp) = date('now', '-1 day')");
      const weekMessages = getCount("SELECT COUNT(*) as count FROM messages WHERE date(timestamp) >= date('now', '-7 days')");
      const lastWeekMessages = getCount("SELECT COUNT(*) as count FROM messages WHERE date(timestamp) >= date('now', '-14 days') AND date(timestamp) < date('now', '-7 days')");

      // Get total API calls
      const totalApiCallsRow = db.prepare('SELECT COALESCE(SUM(request_count), 0) as count FROM api_key_usage_logs').get();
      const totalApiCalls = totalApiCallsRow?.count || 0;

      // Get average latency
      const avgLatencyRow = db.prepare('SELECT AVG(last_latency_ms) as avg FROM bot_stats WHERE last_latency_ms > 0').get();
      const avgLatency = Math.round(avgLatencyRow?.avg || 0);
      ...
      const newAgentsThisWeek = getCount("SELECT COUNT(*) as count FROM bots WHERE date(created_at) >= date('now', '-7 days')");
  ...
  fastify.get('/api/stats/agents/performance', async (request, reply) => {
    try {
      ...
      const agents = performanceData.map(bot => ({
        ...
        status: bot.status,
        avatar: bot.avatar,
        providerType: bot.provider_type,
